...
- [ ] 1.4 Copy wizard header icon: `fluentui-magic-wand` → `setupwizard-wizard-48.png` and `setupwizard-wizard-100.png`
- [ ] 1.5 Copy close button icon from `Assets/Views/ConnectionWindow/connection-close-20.png` → `setupwizard-close-20.png`
...
- [ ] 2.5 Implement `[RelayCommand] GoNext()` — validate current step, advance (step 0→1→2)
- [ ] 2.6 Implement `[RelayCommand] GoBack()` — decrement step
- [ ] 2.7 Implement `[RelayCommand] Finish()` — call `SaveToSettings()`, invoke `CloseAction`
- [ ] 2.8 Implement `[RelayCommand] Cancel()` — invoke `CloseAction` without saving
- [ ] 2.9 Implement `SaveToSettings()` — populate `SettingsService.Current.XboxConnection`, call `CryptoService.Obfuscate(Password)`, `SettingsService.Save()`, `XboxDeviceService.Configure()`
...
- [ ] 5.1 In `App.axaml.cs` `InitAfterSplashAsync`, after `main.Show()` and before `splash.Close()`, add: `if (!SettingsService.Current.XboxConnection.IsConfigured)` → create `SetupWizardViewModel`, create window, `ShowDialog`, on Finish with `OpenConnectionAfter==true` call `await mainViewModel.ShowConnectAction!()`
- [ ] 5.2 Wire `vm.CloseAction = () => win.Close()` in the App.axaml.cs lambda (same pattern as CustomInstallWindow)

## 6. Verify

- [ ] 6.1 Build project with `dotnet build XBVault/XBVault.csproj` — ensure zero errors
