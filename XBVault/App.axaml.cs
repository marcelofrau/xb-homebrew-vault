...
public partial class App : Application
{
        ...
        InstalledAppUpdateService updateService)
    {
            ...
            Func<string, Task> openCustomInstallWithFile = async (filePath) =>
        {
            ...
            settingsViewModel.ReconfigureCredentialsRequested = async () =>
            {
                ...
                if (wizardVm.WasCompleted)
                    settingsViewModel.RefreshFromStorage();
            };
...
System.Diagnostics.Process.Start(new System.Diagnostics.ProcessStartInfo(AppUrls.InspectorDocs)
{
    UseShellExecute = true
            ...
            // Stored password present but undecryptable (config from another machine/user or corrupt) —
            // surface it and offer the setup wizard for reconfiguration.
                ...
                && !CryptoService.TryDeobfuscate(SettingsService.Current.XboxConnection.EncryptedPassword, out _))
            {
                var pvm = new ConfirmViewModel(
                    ...
                    "Reconfigure", "Not now");
                var pwin = new Views.ConfirmWindow { DataContext = pvm };
                await pwin.ShowDialog(main);
                if (pvm.Confirmed && settingsViewModel.ReconfigureCredentialsRequested is not null)
                    await settingsViewModel.ReconfigureCredentialsRequested();
            }
        ...
        Views.MobileSplashView splash)
    {
        try
        {
            ...
            await XBVault.Helpers.UIHelpers.RunOnUIAsync(async () =>
            {
                ...
                main.ExplorerContent.DataContext = fileExplorerViewModel;
                ...
                toolsViewModel.ShowCustomInstallAction = () => ShowMobileCustomInstall(main, authService, packageService, installService).FireAndForget("App.ShowMobileCustomInstall");
                browseViewModel.ShowCustomInstallAction = () => ShowMobileCustomInstall(main, authService, packageService, installService).FireAndForget("App.ShowMobileCustomInstall");
                browseViewModel.OpenUrlAction = url => PlatformHelper.OpenUrl(url);
                ...
                settingsViewModel.ReconfigureCredentialsRequested = () =>
                {
                    Dispatcher.UIThread.Post(() =>
                    {
                        var wizardVm = new SetupWizardViewModel(authService);
                ...
                installedViewModel.ConfirmReinstallAsync = async pkg =>
                {
                    var vm = new Views.MobileConfirmDialogViewModel
                    {
                        ...
                        Message = $"Reinstall \"{pkg.Name}\"? This will reinstall over the existing installation and keep your app data.",
                ...
                browseViewModel.ReinstallFromDetailAction = pkg =>
                    installedViewModel.ReinstallPackageCommand.ExecuteAsync(pkg);
                installedViewModel.ShowCustomInstallAction = () => ShowMobileCustomInstall(main, authService, packageService, installService).FireAndForget("App.ShowMobileCustomInstall");
                ...
                // ── Mobile file explorer: wire action delegates ──
                fileExplorerViewModel.ShowDeleteConfirmAsync = async entries =>
                {
                ...
                fileExplorerViewModel.ShowConnectionInfoAsync = async (host, user, password, port) =>
                {
                    var vm = new Views.MobileSftpInfoViewModel
                    {
                        Host = host,
                        User = user,
                        Password = password,
                    ...
                    var view = new Views.MobileSftpInfoView { DataContext = vm };
                    view.BackRequested += (_, _) => main.CloseOverlay();
                    main.ShowOverlay(view);
                ...
                fileExplorerViewModel.ShowErrorDialog = (title, description, details) =>
                {
                    var vm = new Views.MobileInfoDialogViewModel
                    {
                        Title = title,
                    ...
                    dlg.SetOnBack(() => { vm.OkCommand.Execute(null); main.CloseOverlay(); });
                    main.ShowOverlay(dlg);
                    _ = tcs.ContinueWith(_ => Avalonia.Threading.Dispatcher.UIThread.Post(() => main.CloseOverlay()));
                };

                fileExplorerViewModel.ShowInputDialogAsync = async (title, message, defaultValue, iconUri) =>
                {
                    var vm = new Views.MobileInputDialogViewModel
                    {
                        Title = title,
                        Message = message,
                        Value = defaultValue
                    };
                    var dlg = new Views.MobileInputDialogView { DataContext = vm };
                    var tcs = vm.WaitForResult();
                    dlg.BackRequested += (_, _) => { vm.CancelCommand.Execute(null); main.CloseOverlay(); };
                    main.ShowOverlay(dlg);
                    var result = await tcs;
                    main.CloseOverlay();
                    return result;
                ...
                fileExplorerViewModel.ShowSaveFileDialogAsync = async entry =>
                {
                    var topLevel = TopLevel.GetTopLevel(main)!;
                    var file = await topLevel.StorageProvider.SaveFilePickerAsync(new FilePickerSaveOptions
                    {
                ...
                fileExplorerViewModel.PostDownloadSaveAsync = async (tempPath) =>
                {
                        ...
                        !string.IsNullOrEmpty(tempPath) && File.Exists(tempPath))
                    {
                        try
                        {
                            ...
                            var storageFile = (Avalonia.Platform.Storage.IStorageFile)safFile;
                ...
                fileExplorerViewModel.ShowFolderPickerAsync = async () =>
                {
                    var topLevel = TopLevel.GetTopLevel(main)!;
                ...
                fileExplorerViewModel.ScrollToEntry = _ => { };
                fileExplorerViewModel.FocusFileList = () => { };

                fileExplorerViewModel.OpenCustomInstallWithFileAction = filePath =>
                {
                    ShowMobileCustomInstallWithFile(main, packageService, installService, filePath);
                    return Task.CompletedTask;
                };
                    ...
                    && !CryptoService.TryDeobfuscate(SettingsService.Current.XboxConnection.EncryptedPassword, out _))
                {
                    ...
                    var pvm = new Views.MobileConfirmDialogViewModel
                    {
                    ...
                    var pdlg = new Views.MobileConfirmDialogView { DataContext = pvm };
                    var ptcs = pvm.WaitForResult();
                    main.ShowOverlay(pdlg);
                    pdlg.SetOnBack(() => { pvm.CancelCommand.Execute(null); main.CloseOverlay(); });
                    var confirmed = await ptcs;
                    main.CloseOverlay();
                    if (confirmed && settingsViewModel.ReconfigureCredentialsRequested is not null)
                        _ = settingsViewModel.ReconfigureCredentialsRequested();
                }
            });
        }
        catch (Exception ex)
    ...
    private static async Task ShowMobileCustomInstall(
        ...
        PackageInstallService installService)
    {
        if (!authService.IsConnected)
        {
            ...
            var tcs = new TaskCompletionSource<bool>();
            var connVm = new ConnectionViewModel(authService, null!);
            var connView = new Views.MobileConnectionView { DataContext = connVm };
            connVm.Completed += success => tcs.SetResult(success);
            connView.SetOnBack(() =>
            {
            ...
            var pickFileFilter = new List<FilePickerFileType>
        {
            ...
            vm.CloseAction = () => { main.CloseOverlay(); vm.Dispose(); };
            var ciView = new Views.MobileCustomInstallView();
            ciView.SetViewModel(vm);
            ciView.CloseRequested += (_, _) => { main.CloseOverlay(); vm.Dispose(); };
        ...
        catch (Exception ex)
        {
            ...
            Dispatcher.UIThread.Post(() =>
                ShowErrorDialogSafe("Custom Install Error",
